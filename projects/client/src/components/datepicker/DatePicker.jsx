...
function DatePicker({ selectedDays, handleDayClick, today, asreport = false, price = null, specialPrice = null, dateSpecialPrice = [] }) {
  const isMobile = useCheckMobileScreen();
  const getPriceFormat = (price, date) => {
    let check = false
    dateSpecialPrice.map(dates => {
      if ((isAfter(date, dates.start) && isBefore(date, dates.end)) ||
        (isSameDay(date, dates.start) || isSameDay(date, dates.end))) {
        check = true
      }
    })
    let devide = 1000000
    ...
    const format = check ? `${specialPrice / devide2} ${specialPrice < 1000000 ? " rb" : " jt"}` : `${price / devide} ${price < 1000000 ? " rb" : " jt"}`
    return format
  }
  ...
  return (
    <DayPicker
      ...
      components={price != null || specialPrice != null ? {
        DayContent: dateTime
      } : null
      }
      classNames={{
